...
def clean_and_verify(raw_html, target, report_file, source_label):
    """Sirf wahi data dikhayega jo target se match kare ya SURE_HITS pattern mein ho"""
    try:
        ...
        for line in lines:
            line = line.strip()
            # Strict Filter: Target hona chahiye AUR koi ek ID pattern match hona chahiye
...
def silent_tool_runner(cmd, name, report_file):
    """Faltu ki lines ko filter karke sirf 'FOUND' results dikhayega"""
    try:
        # Torsocks ke saath command run karein
        process = subprocess.Popen(f"torsocks {cmd}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        for line in process.stdout:
            clean = line.strip()
            # Sirf tabhi dikhaye jab HTTP link ho ya 'found' likha ho, aur noise na ho
...
def main():
    ...
    print(f"{Fore.CYAN}╔══════════════════════════════════════════════════════════════╗")
    print(f"{Fore.RED}║    KHALID SHADOW BUREAU - ACCURATE MODE v74.0            ║")
    print(f"{Fore.CYAN}╚══════════════════════════════════════════════════════════════╝")
    ...
    print(f"{Fore.BLUE}[*] Filtering Noise... Extracting Accurate Data Only...\n")
    ...
    print(f"\n{Fore.GREEN}[➔] Scan Complete. Accurate Data Saved: {report_path}")
